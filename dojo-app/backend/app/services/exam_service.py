...
from app.models import (
    Exam, ExamParticipant, ExamBoardMember, Event, Belt, Student,
    BeltPromotion, Attendance,
)
from app.schemas import (
    ExamCreate, ExamUpdate, ExamParticipantCreate, 
    ExamParticipantUpdate, ExamBoardMemberCreate
)
...
class ExamService:
    ...
    @staticmethod
    def get_exams(db: Session, status: Optional[str] = None, skip: int = 0, limit: int = 100) -> list[Exam]:
        query = db.query(Exam)
        if status:
    ...
    def add_participant(
        ...
        existing_participant = db.query(ExamParticipant).filter(
        ...
        if participant_data.role == "candidate":
            is_eligible, reasons = ExamService._check_eligibility(
                db, participant_data.student_id, exam.belt_id
            )
    ...
    @staticmethod
    def update_participant(db: Session, participant_id: str, participant_data: ExamParticipantUpdate) -> ExamParticipant:
    ...
    def promote_candidate(
        db: Session, participant_id: str, new_belt_id: str, promoted_by: str
    ) -> ExamParticipant:
