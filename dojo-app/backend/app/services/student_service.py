...
from app.models import Student, Belt, Attendance, Dojo, BeltPromotion, ExamParticipant, Exam
...
class StudentService:
    @staticmethod
    def _generate_registration_number(db: Session, dojo_id: Optional[str] = None) -> str:
    ...
    @staticmethod
    def get_student_by_registration(db: Session, registration_number: str) -> Optional[Student]:
    ...
    @staticmethod
    def create_student(db: Session, student_data: StudentCreate, created_by: Optional[str] = None) -> Student:
        # Validate belt exists
        belt = db.query(Belt).filter(Belt.id == student_data.current_belt_id).first()
        if not belt:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid belt_id"
            )
        ...
        if student_data.registration_number:
            ...
            registration_number = StudentService._generate_registration_number(
                db, student_data.dojo_id
            )
    ...
    @staticmethod
    def get_last_promotion(db: Session, student_id: str) -> Optional[BeltPromotion]:
        """Get the most recent belt promotion for a student."""
        return db.query(BeltPromotion).filter(
            BeltPromotion.student_id == student_id
    ...
    def get_student_progress(db: Session, student_id: str) -> dict:
        ...
        # Get next belt
        next_belt = db.query(Belt).filter(
            Belt.category == current_belt.category,
            Belt.sort_order > current_belt.sort_order
        ).order_by(Belt.sort_order).first()
    ...
    def get_student_exam_history(db: Session, student_id: str) -> list[dict]:
        ...
        participations = db.query(ExamParticipant).filter(
            ExamParticipant.student_id == student_id
        ).all()
