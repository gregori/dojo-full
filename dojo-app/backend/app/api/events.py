...
def create_event_type(
    event_type_data: EventTypeCreate, db: Session = Depends(get_db), current_user=Depends(get_current_admin)
):
...
def delete_event_type(event_type_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_admin)):
...
def get_event(event_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_instructor_or_admin)):
    """Get event by ID."""
...
def create_event(
    event_data: EventCreate, db: Session = Depends(get_db), current_user=Depends(get_current_instructor_or_admin)
):
    """Create a new event."""
...
def cancel_event(event_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_instructor_or_admin)):
    """Cancel event."""
...
def get_event_qr_code(
    event_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_instructor_or_admin)
):
