...
### All 3 failures fixed and confirmed via real CI (2026-07-22)

1. **PDF success-message timeout:** not a UI-timing issue. `MedicalExamPage.tsx` did `response.data.message || <Portuguese fallback>`, but the public endpoint always returns a non-empty generic message by design (never disclose credential validity), so the Portuguese fallback the test's regex expects was unreachable — the backend's English generic text was shown instead. Fixed by always showing the fixed Portuguese message and ignoring the response body (`dojo-app/frontend/src/pages/MedicalExamPage.tsx`).
2. **JPEG/PNG 400s:** the leading hypothesis above (Cypress `selectFile()` sending a wrong `Content-Type`) was disproven — captured the real outgoing browser request via `cy.intercept` and confirmed the headers and magic bytes were correctly formed. The actual cause: `cy.intercept()` decodes/re-encodes intercepted request bodies as UTF-8 text before forwarding, which corrupts binary multipart content (byte sequences that aren't valid UTF-8 — like these JPEG/PNG magic bytes — become replacement characters on the wire), so the backend correctly rejects the now-corrupted upload with 400. PDF was unaffected only because its content is pure ASCII, which round-trips through UTF-8 unchanged. Fixed by removing the `cy.intercept()`/`cy.wait()` pattern for the JPEG/PNG tests and verifying success via the UI message instead, matching PDF's own check (`medical-exam.cy.ts`).
3. **Bonus, previously masked:** once both real bugs were fixed, the `e2e` job still went red — `actions/setup-python`'s post-job cache-save step failed because nothing in that job runs `pip` (docker compose builds the backend in its own container), so the cache directory it tried to save never existed. Removed the unused Python setup step from `.github/workflows/ci-frontend.yml`.

**Confirmed via real CI, run [29915208590](https://github.com/gregori/dojo-full/actions/runs/29915208590): 25/25 Cypress tests passing (`precheckin.cy.ts` 5/5, `medical-exam.cy.ts` 10/10, `financial.cy.ts` 10/10), `e2e` job green.**

Local reproduction on this session's (Windows) sandbox was unreliable for an unrelated reason: this machine's Docker Desktop/WSL2 port-forwarding intermittently corrupts some `localhost`-routed POST bodies, plus two now-fixed local-only issues (`entrypoint.sh` CRLF corruption on checkout — fixed via a new `.gitattributes`; a stray host `.venv` contaminating the container via the bind mount — cleaned up). The JPEG/PNG root cause was ultimately confirmed by iterating against real CI runs rather than continuing to fight the local sandbox.
